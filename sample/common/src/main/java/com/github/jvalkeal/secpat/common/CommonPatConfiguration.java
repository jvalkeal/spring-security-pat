...
public class CommonPatConfiguration {
	...
	public PatAuthorizationService patAuthorizationService() {
		...
		service.save(PatAuthorization.builder().principal("user1").scopes(new HashSet<>(Arrays.asList("read"))).token(generator.apply("user1")).issuedAt(now).expiresAt(month).notBefore(now).build());
		service.save(PatAuthorization.builder().principal("user2").scopes(new HashSet<>(Arrays.asList("write"))).token(generator.apply("user2")).issuedAt(now).expiresAt(month).notBefore(now).build());
		service.save(PatAuthorization.builder().principal("user3").scopes(new HashSet<>(Arrays.asList("write"))).token(generator.apply("user3")).issuedAt(now).expiresAt(sec).notBefore(now).build());
		service.save(PatAuthorization.builder().principal("user4").scopes(new HashSet<>(Arrays.asList("write"))).token(generator.apply("user4")).issuedAt(now).expiresAt(month).notBefore(monthMinusDay).build());
		service.save(PatAuthorization.builder().principal("user5").scopes(new HashSet<>(Arrays.asList("read"))).token(generator.apply("user5")).issuedAt(nowMinus2Days).expiresAt(nowMinus1Day).notBefore(nowMinus2Days).build());
		String token6 = uuidPatService.generator().apply(null);
		service.save(PatAuthorization.builder().principal("user6").scopes(new HashSet<>(Arrays.asList("read"))).token(token6).issuedAt(now).expiresAt(month).notBefore(now).build());
		String token7 = checksumPatService.generator().apply(null);
		service.save(PatAuthorization.builder().principal("user7").scopes(new HashSet<>(Arrays.asList("read"))).token(token7).issuedAt(now).expiresAt(month).notBefore(now).build());
