...
public class ApiServerController {
	...
	private final static Logger log = LoggerFactory.getLogger(ApiServerController.class);
	...
	@GetMapping("/whoami4")
	String whoami4(@AuthenticationPrincipal User user) {
		log.debug("User Class {}", user != null ? user.getClass() : null);
		log.debug("User {}", user);
		return String.format("%s - %s", user != null ? user.getClass() : null, user);
	}
	...
	@GetMapping("/read")
	@Secured("read")
	String read(Authentication user) {
		log.debug("User Class {}", user != null ? user.getClass() : null);
		log.debug("User {}", user);
		return String.format("%s - %s", user != null ? user.getClass() : null, user);
	}
	...
	@GetMapping("/write")
	String write(Authentication user) {
		log.debug("User Class {}", user != null ? user.getClass() : null);
		log.debug("User {}", user);
		return String.format("%s - %s", user != null ? user.getClass() : null, user);
	}
